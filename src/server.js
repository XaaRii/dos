...
/// REPL
const repl = require('repl'), r = repl.start({ prompt: '', useColors: true });
r.context.debug = debug;
r.context.server = server;
r.context.io = io;
r.context.lobbySystem = lobbySystem;
r.context.sock = [];
if (debug) r.context.sockLobby = sockLobby;
if (debug) r.context.gameLobby = gameLobby;
///
...
io.on('connection', (socket) => {
	...
	// Game control
	socket.on('serverGameStart', async () => sockGameStart(socket));
	socket.on('serverGameRestart', async () => sockGameRestart(socket));
	socket.on('serverGameUpdate', async (action, sec) => sockGameUpdate(socket, action, sec));
	socket.on('serverGameStop', async () => sockGameStop(socket));

	// Lobby control
	socket.on('serverRefreshLobby', async () => sockRefreshLobby(socket));
	...
	socket.on('disconnect', async () => {
		// must be inside io.connection, otherwise socket is undefined
...
async function sockGameStart(socket) {
...
async function sockGameRestart(socket) {
...
async function sockGameUpdate(socket, action, sec) {
	...
	try {
		...
		switch (action) {
			case "playCard": // sec = cardIndex in hand
				...
				switch (chosenX) {
					case 0: playSpecialCard("black", currentGame, socket); break;
					case 1: playSpecialCard("blackfour", currentGame, socket); break;
...
async function sockGameStop(socket) {
...
async function sockRefreshLobby(socket) {
	if (debug) console.log("serverRefreshLobby");
	if (debug) console.log(lobbySystem.getLobbyList());
	socket.emit('clientLobbyList', lobbySystem.getLobbyList());
};

async function sockCreateLobby(socket, lobbyName, username) {
	...
	try {
		...
		sockJoinLobby(socket, newLobby._id, username);
	} catch (err) {
		console.error('Error creating lobby:', err);
	}
};

async function sockJoinLobby(socket, lobbyId, username) {
...
async function sockLeaveLobby(socket, who) {
...
async function playSpecialCard(type, currentGame, socket) {
	if (debug) console.log("playSpecialCard", type);
	switch (type) {
		case "black":
			colorPicker(socket, currentGame, false);
			break;

		case "blackfour":
			colorPicker(socket, currentGame, true);
			break;
...
function colorPicker(socket, currentGame) {
...
async function drawCards(currentGame, times, animation) {
	...
	for (let i = 0; i < times; i++) {
		...
		let drawnCard = await currentGame.drawPile.shift();
		if (!drawnCard) {
			currentGame.drawPile = shuffle([...validCardSet]);
			drawnCard = await currentGame.drawPile.shift();
		}
...
function handUpdate(currentGame) {
	io.to(currentGame.playerIDs[currentGame.currentPlayerIndex].sid).emit('clientGameUpdate', null, currentGame.hands[currentGame.currentPlayerIndex]);
